...
/**
 * A question may not depend on one a shallower mode never asks.
 *
 * `requires` and interpolation both make a question wait for another's answer.
 * If the question waited on is deeper, the waiting question is unreachable in
 * every mode that could otherwise have asked it — a tier-1 question gated on a
 * tier-2 answer simply never appears in Quick, silently, and the modifier tag
 * it was written to feed never gets earned.
 */
function checkTierDependencies(ctx: Ctx): void {
  const report = (question: NormalisedQuestion, targetId: string, how: string): void => {
    const target = ctx.content.questionById.get(targetId);
    if (!target || target.depth <= question.depth) return;
    ctx.push({
      severity: 'error',
      code: 'tier/depends-on-deeper',
      file: FILES.questions,
      id: question.id,
      line: lineOf(ctx, 'questions', question.id, []),
      message: `tier-${question.depth} question ${how} tier-${target.depth} question "${targetId}"`,
      hint: 'a mode that asks this one may never ask the one it waits for, so it can never be reached. Move this question deeper, or make it stand on its own',
    });
  };

  for (const question of ctx.content.questions) {
    for (const targetId of conditionQuestionIds(question.requires)) {
      report(question, targetId, 'is gated on');
    }
    for (const field of ['text', 'tooltip'] as const) {
      const text = question[field];
      if (!text) continue;
      for (const match of text.matchAll(/\{\{\s*answers\.([a-z][a-z0-9_]*)\./g)) {
        report(question, match[1] as string, 'interpolates');
      }
    }
  }
}

/** Every question id named by an `answered` atom anywhere in a condition. */
function conditionQuestionIds(condition: Condition | undefined): string[] {
  if (!condition) return [];
  if ('all' in condition) return condition.all.flatMap(conditionQuestionIds);
  if ('any' in condition) return condition.any.flatMap(conditionQuestionIds);
  if ('not' in condition) return conditionQuestionIds(condition.not);
  if ('answered' in condition) return Object.keys(condition.answered);
  return [];
}
